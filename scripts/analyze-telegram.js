 ...
 * Analyzes chat history to extract dispatcher decision patterns and communication style
 *
 * Usage:
 *   1. Set ANTHROPIC_API_KEY in environment
 *   2. npm install @anthropic-ai/sdk  (if not installed)
 *   3. node scripts/analyze-telegram.js
 */
...
const CHUNK_SIZE = 150; // messages per chunk sent to Claude

// Keywords that identify dispatcher-relevant messages
const KEYWORDS = [
  // English
...
{
  "decisions": [
    { "situation": "short description", "action": "take|reject|negotiate", "reason": "why", "quote": "exact text from message" }
  ],
  "pricing_rules": ["rule extracted from messages, e.g. 'Below $1.80 RPM is considered bad'"],
  "broker_notes": [
    { "broker": "broker name or ID", "note": "opinion/observation", "quote": "exact text" }
  ],
  "lane_notes": [
    { "lane": "state or city pair", "note": "good/bad/risky and why", "quote": "exact text" }
  ],
  "style_samples": ["exact short quotes showing how decisions are communicated"],
  "rules": ["any recurring rules, policies, or advice mentioned"]
}
...
async function generateSystemPrompt(client, patterns) {
  console.log('\nGenerating system prompt from patterns...');
  ...
  const response = await client.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `You have analyzed hundreds of messages from a trucking dispatcher.
Based on these extracted patterns, write a system prompt that makes an AI assistant think and talk like this dispatcher.

The system prompt must:
1. Capture the decision-making logic (when to take/reject/negotiate a load)
2. Reflect the pricing philosophy (what RPM is good/bad)
3. Include real opinions about brokers and lanes from the data
4. Match the communication style (direct, short, mix of Ukrainian/English where natural)
5. Include 5-8 concrete few-shot examples from the actual quotes
6. Sound like an experienced human dispatcher, NOT an AI analyst

Write the system prompt in English. Make it practical and specific, not generic.
...
async function main() {
  ...
  console.log(`Total messages in file: ${all.length.toLocaleString()}`);

  // ── Filter relevant ──
  const relevant = all.filter(isRelevant);
  console.log(`Dispatcher-relevant messages: ${relevant.length.toLocaleString()}`);

  if (relevant.length === 0) {
    console.error('No relevant messages found. Check keywords in config.');
    process.exit(1);
  }
  ...
  const estimatedCost = (chunks.length * 0.08).toFixed(2);
  ...
  console.log(`📊 Patterns file:   scripts/dispatcher_patterns.json`);
  console.log(`🧠 System prompt:   scripts/system_prompt_chat.txt`);
  console.log('\nExtracted:');
  ...
  console.log(`  General rules:  ${patterns.rules.length}`);
}
