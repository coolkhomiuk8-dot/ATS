...
export async function answerCallStatsQuestion(question) {
  ...
  const yesterdaySnap = await getYesterdaySameTimeSnapshot();
  const nowET = new Date().toLocaleTimeString("uk-UA", { timeZone: "America/New_York", hour: "2-digit", minute: "2-digit" });
  const sameTimeLine = yesterdaySnap
    ? `ВЧОРА СТАНОМ НА ЦЮ Ж ГОДИНУ (~${nowET} за Нью-Йорком):\n` +
      yesterdaySnap.stats.map((s) => `${s.name} (${s.account}): ${s.callCount} дзвінків`).join("\n")
    : `ВЧОРА СТАНОМ НА ЦЮ Ж ГОДИНУ: даних немає (історія ще накопичується).`;

  const context =
    `СЬОГОДНІ (поточний момент, ${nowET} за Нью-Йорком):\n${fmt(today)}\n\n` +
    `ВЧОРА (весь день, повний підсумок):\n${fmt(yesterday)}\n\n` +
    `${sameTimeLine}`;
      ...
      messages: [
        {
          role: "user",
          content:
            `Ти — асистент диспетчерської компанії. Нижче — дані про дзвінки команди: сьогодні (станом на зараз, ` +
            `не кінець дня — врахуй, скільки часу реально минуло), учора повністю за весь день, і учора станом ` +
            `на цю ж годину (якщо є) — використовуй САМЕ цей останній варіант, коли питання про темп/динаміку ` +
            `"на цей момент", а не весь учорашній день.\n` +
