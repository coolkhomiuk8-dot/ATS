    ...
    >
      ...
      <div style={{ minWidth: 180, flexShrink: 0, padding: "0 16px", borderRight: "1px solid var(--border)" }}>
        ...
        {truck.samsaraId && (() => {
          ...
          const snapshots = (truck.fuelHistory || []).length;
          ...
          if (mpg != null) {
            ...
            return (
              <div title={`7d: ${c.mpg7d ?? "—"} mpg · ${c.gallons7d || 0} gal used\n30d: ${c.mpg30d ?? "—"} mpg · ${c.gallons30d || 0} gal used`}
          ...
          return (
            <div title={`Tracking fuel consumption — ${snapshots} snapshot${snapshots !== 1 ? "s" : ""} so far. MPG appears after enough driving with fuel changes.`}
              style={{ marginTop: 6, display: "inline-flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 600, padding: "2px 7px", borderRadius: 5, background: "var(--bg-hover)", color: "var(--text-faint)", border: "1px dashed var(--border)", cursor: "default" }}
            >
              ⛽ tracking… ({snapshots})
            </div>
          );
